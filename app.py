...
INPUT_FILE = "audio/input/input.wav"
app = Flask(__name__)
...
def synthesize(text):
    ...
    tts = TTS(model_name='tts_models/en/ljspeech/vits', progress_bar=False).to(device) #possible models listed below
    # Run TTS
    tts.tts_to_file(text=line, file_path=OUTPUT_FILE)

    #voice cloining from 6 seconds of audio example
    #tts = TTS("tts_models/multilingual/multi-dataset/xtts_v2").to(device)
    # Text to speech to a file
    #tts.tts_to_file(text=line, speaker_wav=INPUT_FILE, language="en", file_path=OUTPUT_FILE)
...
def synthesize2():
    ...
    text = data['text']
    #voice = data.get('voice', 'tts_models/en/ljspeech/vits')
   # device = "cuda" if torch.cuda.is_available() else "cpu"
    device = "cpu"
    # Init TTS with the target model name
    tts = TTS(model_name='tts_models/en/ljspeech/vits', progress_bar=False).to(device) 
    # Run TTS
    wav = tts.tts_to_file(text=text, file_path=OUTPUT_FILE)
    enc = base64.b64encode(open(OUTPUT_FILE, "rb").read())
    return(enc) 
